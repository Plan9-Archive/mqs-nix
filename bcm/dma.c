...
dmastart(int chan, int dev, int dir, void *src, void *dst, int len)
{
	...
	case DmaD2M:
		...
		ti = Srcdreq | Destinc;
		cb->sourcead = DMAADDR(src) | 0x7E000000;
		cb->destad = PADDR(dst) | 0x40000000;
		break;
	case DmaM2D:
		cachedwbse(src, len);
		ti = Destdreq | Srcinc;
		cb->sourcead = PADDR(src) | 0x40000000;
		cb->destad = DMAADDR(dst) | 0x7E000000;
		break;
	case DmaM2M:
		cachedwbse(src, len);
		cachedwbinvse(dst, len);
	...
	ctlr->regs[Cs] = 0;
	microdelay(1);
	ctlr->regs[Conblkad] = PADDR(cb) | 0x40000000;
