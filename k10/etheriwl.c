...
readfirmware(char *name)
{
	...
	r = 0;
	while(r < d.length){
		n = c->dev->read(c, data+r, d.length-r, (vlong)r);
		if(n <= 0)
			break;
		r += n;
	}
...
boot(Ctlr *ctlr)
{
	...
	p += ROUNDUP(fw->init.data.size, 16);
	...
	size = ROUNDUP(fw->main.data.size, 16) + ROUNDUP(fw->main.text.size, 16);
	dma = mallocalign(size, 16, 0, 0);
	if(dma == nil)
		return "no memory for dma";
	if((err = niclock(ctlr)) != nil){
	...
	p += ROUNDUP(fw->main.data.size, 16);
...
iwlpci(void)
{
	...
	once = 1;
	p = &iwlhead;
	for(pdev = nil; pdev = pcimatch(pdev, 0, 0); ) {
		...
		if(fwname[ctlr->type] == nil){
			print("etheriwl: unsupported controller type %d\n", ctlr->type);
