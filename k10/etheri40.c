...
multicast(void *a, uchar *ea, int on)
{
	...
	if(rv != 0){
		snprint(buf, sizeof buf, "%s: multicast: error %d", cname(c), rv);
		if(rv < nelem(errtab))
			snprint(buf, sizeof buf, "%s: multicast: %s", cname(c), errtab[rv]);
		error(buf);
	}
}
...
detach(Ctlr *c)
{
	...
	pf = c->pf;
	wait = (c->reg[Rstctl + pf] & 0x3f)*100;
	for(i = 0;; i++){
		if(i == wait)
			return -1;
		if((c->reg[Rststat + pf] & Devstate) == 0)
			break;
		delay(1);
	}
	if(i == 0){
...
hmcmap(Ctlr *c, u64int fpa)
{
	...
	pgno = fpa>>Fpgshift;
	if(pgno >= nelem(c->pagetab))
		panic("%s: hmcmap: pgno %d > %d", cname(c), pgno, nelem(c->pagetab));
	if(c->pagetab[pgno] != nil)
		return;
	c->pagetab[pgno] = mallocalign(Hmcpgsz, Hmcalign, 0, 0);
}
...
hmcfault(Ctlr *c, u64int fpa)
{
	...
	hmcmap(c, fpa);
	pgno = fpa>>Fpgshift;
	c->pdpage[pgno] = Pciwaddr(c->pagetab[pgno]) | Fpdevalid;
}

#define Pageno(x)	((x)>>Fpgshift)
...
hmcinit(Ctlr *c)
{
	 ...
	 */
	c->reg[Hmsdaddrhi] = Pciwaddrh(c->pdpage);
	c->reg[Hmsdaddrlo] = Pciwaddrl(c->pdpage) | 1*Sdcount | Sdvalid | Sdpaged;
	c->reg[Hmsdcmd] = Hmcmdwrite | Pageno(0);
}
...
irqenable(Ctlr *c)
{
	...
	for(pf = 0; pf < c->npf; pf++){
		c->reg[Irxqctl + 0] = Irxidx*Iqirqidx | 0*Iqnxtqidx | Iqnxtt | Iqenable;	/* rx: ics=0x04 */
		c->reg[Itxqctl + 0] = Itxidx*Iqirqidx | Iqend | Iqenable;		/* tx: ics=0x02 */
		c->reg[Ilnklst + pf] = 0*Firstqidx | Firstrx;				/* 1st: rxq idx 0 */

		c->reg[Icren + pf] = c->im;
		c->reg[Idynctl + pf] = Intena | Clearpba;
	}
	im(c, Errint);
}
...
attach(Ether *e)
{
	...
	snprint(buf, sizeof buf, "#l%da", e->ctlrno);
	kproc(buf, aproc, e);
...
interrupt(Ureg*, void *v)
{
	...
	icr = c->reg[Icr + pf] & c->im;
	c->reg[Idynctl + pf] = Intena | Clearpba;

//	iprint("%s: irq %.8ux im %.8ux\n", cname(c), icr, c->im);
