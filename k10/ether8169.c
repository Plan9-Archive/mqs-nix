...
rtl8169attach(Ether* edev)
{
	...
	/*
	 * Wait for link to be ready.  why here?
	 */
	if(firsta){
		for(timeo = 0; timeo < 350; timeo += 10){
			if(miistatus(ctlr->mii) == 0)
				break;
			tsleep(&up->sleep, return0, 0, 10);
		}
		phy = ctlr->mii->curphy;
		dprint("%s: speed %d fd %d link %d rfc %d tfc %d\n",
...
rtl8169link(Ether* edev)
{
	...
	phy = ctlr->mii->curphy;
	if(miistatus(ctlr->mii) < 0){
		dprint("%slink n: speed %d fd %d link %d rfc %d tfc %d\n",
	...
	if(phy->speed == 10){
		...
		edev->mbps = 1000;
	dprint("%slink y: speed %d fd %d link %d rfc %d tfc %d\n",
		edev->name, phy->speed, phy->fd, phy->link,
		phy->rfc, phy->tfc);
