...
#define get16(p)		((u32int)getle((p), 2))
#define get32(p)		((u32int)getle((p), 4))
...
static u32int
apicmkintr(uint src, uint inttype, int polarity, int trigger, uint apicno, uint intin)
{
	u32int v;
	Apic *apic;
	...
	if(apicno != 0xff){
		...
		default:
			...
			return 0;
		case Iointr:
			if((apic = ioapiclookup(apicno)) == nil){
				print("ioapic%d: ioapic unusable\n", apicno);
				return 0;
			}
			if(intin >= apic->nrdt){
				print("ioapic%d: intin %d >= nrdt %d\n", apicno, intin, apic->nrdt);
				return 0;
			}
			break;
		case Lintr:
			if((apic = lapiclookup(apicno)) == nil){
				print("lapic%d: lapic unusable\n", apicno);
				return 0;
			}
			if(intin >= nelem(apic->lvt)){
...
{
	uint apicno, intin, polarity, trigger;
	u32int i;
...
enumprt(void *dot, void *)
{
	...
	for(i=0; i<n; i++){
		...
		gsi = amlint(b[3]);
		if(gsi == 0){
			print("acpi: gsi is zero ... setuplink\n");
...
parsesrat(Tbl *t, Parsedat*)
{
	...
	for(p = t->data + 12; p < e; p += p[1]){
		...
		case 0:		/* apic */
			if(get32(p+4)&1)
				lapicsetcolor(p[3], p[2] | p[9]<<24| p[10]<<16 | p[11]<<8);
			break;
		case 1:
			if(get32(p+28)&1)
				adrsetcolor(get64(p+8), get64(p+16), get32(p+2));
			break;
		case 2:		/* x2apic */
			if(get32(p+12)&1)
				lapicsetcolor(get32(p+8), get32(p+4));
			break;
		default:
...
static long
readfadt(Chan*, void *a, long n, vlong o)
{
	char *s, *p, *e;
	Fadt *f;
...
static void
parsehpet(Tbl *t, Parsedat*)
{
	int minticks;
	Hpet *h;
	Gas g;
	extern void hpetinit(int, uintmem, int);
...
//	hpetinit(h->seqno, g.addr, minticks);
	USED(minticks);
}
...
iapcbootarch(void)
{
	...
	sys->nolegacyprobe = !(i&Blegacy);
	sys->noi8042kbd = !(i&B8042kbd);
	sys->novga = i&Bnovga;
	sys->nomsi = i&Bnomsi;
	sys->nomsix = sys->nomsi;
	sys->nocmos = i&Bnocmos;
}
...
#define acpiinit(x)	mpsinit(x)	/* temporary hack */
void
