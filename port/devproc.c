...
procread(Chan *c, void *va, long n, vlong off)
{
	...
	if(QID(c->qid) == Qtrace){
		...
		while(navail > 0) {
			...
			rptr += BIT64SZ;
			PBIT32(rptr, tevents[tconsumed & Emask].machno);
			rptr += BIT32SZ;
			tconsumed++;
			navail--;
		}
	...
	case Qmem:
		...
		if(offset <= USTKTOP)
			...
			r = procctlkmemio(p, offset, n, va, 1);
		poperror();
		psdecref(p);
		return r;
	...
	case Qstatus:
		...
		/*
		 * NIX: added # of traps, inters and syscalls
		 */
...
procctlreq(Proc *p, char *va, int n)
{
	...
	if(p->kp && !kpctl[ct->index])		/* restructed ctl requests to kprocs */
		error(Eperm);
...
procctlkmemio(Proc*, uintptr offset, int n, void *va, int read)
{
	...
	kva = nil;
	if(offset >= KZERO && offset < PTR2UINT(end)) {
		if(offset+n > PTR2UINT(end))
			n = PTR2UINT(end) - offset;
		kva = (void*)offset;
	}
	if(offset >= KZERO && offset < PTR2UINT(sys->vmend)){
		if(offset+n > PTR2UINT(sys->vmend))
			n = PTR2UINT(sys->vmend) - offset;
		kva = (void*)offset;
	}
