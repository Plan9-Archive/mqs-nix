...
ibrk(uintptr addr, int seg)
{
	...
	if(seg == BSEG && newtop >= ROUNDUP(s->top, 1*GiB) + 1*GiB && getpgszi(1*GiB) >= 0){
		DBG("segment using 1G pages\n");
		/*
		 * brk the bss up to the 1G boundary, and create
		 * a new segment above 1G using 1G pages.
		 * This is transparent to the up.
		 */
		rtop = ROUNDUP(newtop,1*GiB);
		newtop = ROUNDUP(s->top, 1*GiB);
		newpages = (newtop-s->base)/pgsz;
		DBG("ibrk: newseg %#p %p (%ld pages)\n", newtop, rtop, newpages);
		ns = newseg(SG_BSS, newtop, rtop);
		ns->color= s->color;
		up->seg[HSEG] = ns;
		DBG("ibrk: newtop %#p newpages %#ulx \n", newtop, newpages);
	...
	mapsize = HOWMANY(newpages, s->ptepertab);
...
syssegflush(Ar0* ar0, va_list list)
{
	...
	uintptr addr;
	Pte *pte;
	usize pgsz, pgshift, chunk, l, len, pe, ps;
	...
	more:
		...
		if(pte)
			pteflush(pte, ps>>pgshift, pe>>pgshift);
