...
static	Header	*rover;
static	uvlong	tailsize;
static	uvlong	tailnunits;
static	Header	*tailbase;
static	Header	*tailptr;
static	Header	checkval;
static	uvlong	morecore(uvlong);
...
/*
 * Experiment: per-mach quick lists.
...
qmallocalign(usize nbytes, uintptr align, long offset, usize span)
{
	...
	if(align < sizeof(Header))
		align = sizeof(Header);
	if(!ISPOWEROF2(align))
		align = 1<<log2ceil(align);
 ...
 * releasing memory into the QLIST.
 */
static void
...
//			p = seprint(p, e, "q%d %d\n", i, n);
	...
	p = seprint(p, e, "quick: %llud bytes total\n", t);
...
//			p = seprint(p, e, "m%d\t%#p\n", q->s.size, q);
	...
	p = seprint(p, e, "total allocated %llud, %llud remaining\n",
		(uvlong)(tailptr-tailbase)*sizeof(Header), tailnunits*sizeof(Header));
...
void*
mallocalign(usize nbytes, usize align, long offset, usize span)
{
	void *v;
...
mallocinit(void)
{
	...
	print("base %#p ptr %#p nunits %llud; %llud mb\n", tailbase, tailptr, tailnunits, (uvlong)(sys->vmend - sys->vmunused)/MiB);
}

static uvlong
morecore(uvlong nunits)
{
	/*
	 * First (simple) cut.
