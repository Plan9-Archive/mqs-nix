...
typedef struct Waitstat	Waitstat;
...
struct Chan
{
	...
	Qid	qid;
	int	fid;			/* for devmnt */
	uint	iounit;			/* chunk size for i/o; 0==default */
...
struct Pgsza
{
	uint	freecount;	/* how many pages in the free list? */
...
struct Proc
{
	...
	Schedq  *rq; 	/* run queue this process is on */
	...
	int	parentpid;
	ulong	time[2*Ntime];	/* User, Sys, Real; child U, S, R */
	...
	uint	procmode;	/* proc device file mode */
	uint	privatemem;	/* proc does not let anyone read mem */
	...
	uint	delaysched;
	uint	priority;	/* priority level */
	uint	basepri;	/* base priority level */
...
struct Perf
{
	uvlong	intrts;		/* time of last interrupt */
	uvlong	inintr;		/* time since last clock tick in interrupt handlers */
	uvlong	avg_inintr;	/* avg time per clock tick in interrupt handlers */
	uvlong	inidle;		/* time since last clock tick in idle loop */
	uvlong	avg_inidle;	/* avg time per clock tick in idle loop */
	uvlong	last;		/* value of perfticks() at last clock tick */
	uvlong	period;		/* perfticks() per clock tick */
};
