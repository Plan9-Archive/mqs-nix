...
sysexec(Ar0* ar0, va_list list)
{
	...
	textlim = ROUNDUP(UTZERO+hdrsz+textsz, 1<<USEGSHIFT);
	datalim = ROUNDUP(textlim+datasz, 1<<USEGSHIFT);
	bsslim = ROUNDUP(textlim+datasz+bsssz, 1<<USEGSHIFT);
